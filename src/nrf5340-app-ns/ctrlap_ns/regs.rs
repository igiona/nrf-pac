...
impl SecureapprotectLock {
    ...
    #[inline(always)]
    pub const fn lock(&self) -> super::vals::SecureapprotectLockLock {
        let val = (self.0 >> 0usize) & 0x01;
        super::vals::SecureapprotectLockLock::from_bits(val as u8)
    }
    #[doc = "Lock register SECUREAPPROTECT.DISABLE from being written until next reset"]
    #[inline(always)]
    pub fn set_lock(&mut self, val: super::vals::SecureapprotectLockLock) {
        self.0 = (self.0 & !(0x01 << 0usize)) | (((val.to_bits() as u32) & 0x01) << 0usize);
    }
}
